...
void SWMesh::setTriangleStream( size_t count, const SWIndex3* stream )
{
	m_triangles.resize(count);
	memcpy( &m_triangles[0], stream, sizeof(SWIndex3)*count );
	m_updateMesh = true;
}
...
void SWMesh::getTriangleStream( tarray<SWIndex3>& stream ) const
{
	stream = m_triangles;
}
...
void SWMesh::setTriangle( tuint index, const SWIndex3& val )
{
...
void SWMesh::getTriangle( tuint index, SWIndex3& val )
{
